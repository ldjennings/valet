...
#figure(
    grid(
        columns: 2,
        gutter: 1em,
        image("media/photos/goal_pose_point.png"),
        image("media/photos/goal_pose_diff.png"),
        image("media/photos/goal_pose_car.png"),
        image("media/photos/goal_pose_trailer.png"),
    ),
...
Of specific note for the car's goal pose is that it is slightly farther out than the other vehicles. Rather than sitting at the center of its grid cell, it instead protrudes into the cells above. Experimentation found that the car's ability to make that maneuver had a cutoff at about .2 grid cells above the standard offset of the other vehicles. Therefore I decided to set the car's offset at .25 grid cells above the standard to avoid needlessly long search times.

#figure(
    image("media/photos/exhaustive_car_6x8_115_fine.png", width: 80%),
    caption: [Exhaustive Hybrid A\* search on a reduced 6×8 grid with fine settings (0.25 XY spacing, π/4 angular spacing, no obstacles) terminating without a valid path after 30,010 expansions. This confirmed that the car requires the additional 0.25 grid cell goal offset to reliably reach its goal — reducing the car footprint was considered and rejected as the worse alternative.]
)
...
#figure(
    grid(
        columns: 2,
        gutter: 1em,
        image("media/photos/point_nav.png"),
        image("media/photos/diff_nav.png"),
        image("media/photos/car_nav.png"),
        image("media/photos/trailer_nav.png"),
    ),
