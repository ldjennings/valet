...
#figure(
    ...
    caption: "Icicle plot rendering of profiled trailer simulation. The runtime of the hybrid A* algorithm is roughly evenly split between collision checking and primitive generation."
)
...
#figure(
    ...
    caption:"Obstacles generated given a 10% occupation requirement. This picture was captured while driving the car in manual mode."
)
...
Obstacles are generated by randomly placing tetrominoes throughout the grid, until a set proportion of the grid is occupied by obstacles. The standard proportion is 10%. The vehicles (green rectangle) start in the top left, and attempts to navigate to the goal pose (yellow rectangle) in the bottom left. The starting and ending positions are always cleared.

// shots of each ending position for the vehicles, make it using a small grid with the vehicle turned off.

Of specific note for the car's goal pose is that it is slightly farther out than the other vehicles. Rather than sitting at the center of its grid cell, it instead protrudes into the open. This was done to increase the success rate.
